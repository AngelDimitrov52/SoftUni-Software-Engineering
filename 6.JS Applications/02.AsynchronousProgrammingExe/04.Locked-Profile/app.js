function lockedProfile() {
    ...
    (async () => {
        ...
        let staticProfile = document.querySelector('.profile');
        staticProfile.remove();

        Object.keys(profiles).forEach((key, i) => {
            let profile = profiles[key];
            let htmlProfile = creteProfile(i + 1, profile.username, profile.email, profile.age);
            mainSection.appendChild(htmlProfile);
        })
    })();

    function creteProfile(userIndex, username, email, age) {
